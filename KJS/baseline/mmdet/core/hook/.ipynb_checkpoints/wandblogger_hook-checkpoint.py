...
class MMDetWandbHook(WandbLoggerHook):
    ...
    def after_train_epoch(self, runner):
        ...
        if self.log_evaluation and self.eval_hook._should_evaluate(runner):
            ...
            # Compare MAP and create new table ##############################
            ap_table, eval_epoch_dict = self._log_class_map(results)
            self.wandb.log(dict(AP = ap_table) , commit = False)
            self.ap_logs.append(eval_epoch_dict)
            #################################################################
    ...
    def after_train_iter(self, runner):
        ...
        if self.log_evaluation and self.eval_hook._should_evaluate(runner):
            ...
            self._init_pred_table()
            # Log predictions
            eval_idx, gt_wandb_bbox = self._log_predictions(results)
            # Log the table
    ...
    def after_run(self, runner):
        length = len(self.ap_logs)
        map_values = [[] for _ in range(length)]
        label_classes = list(self.val_dataset.CLASSES) 
       # 각 class별 AP를 저장
        map_values = [[] for _ in range(len(label_classes))]
        length = len(self.ap_logs)
        
        for epoch in range(length):
            ap_dict = self.ap_logs[epoch]
            for label_num, label_name in enumerate(label_classes):
                ap_value = ap_dict[label_name]
                map_values[label_num].append(ap_value)
            
        map_values = [np.array(value) for value in map_values]

        fig,ax = plt.subplots(5,2, figsize = (60,60))
        fig.suptitle("Multiple class AP", fontsize = 12)
        
        ax = ax.flatten()
        for idx, value in enumerate(map_values):
            #Acc_age_old, Acc_total 이런 느낌으로 2번째 것을 추출하면 충분
            label = label_classes[idx]
            title = f"{label} AP per epoch"
            ...
            ax[idx].plot(x_values, y_values, label = label)
            ax[idx].legend()

            for x, y in zip(x_values, y_values):
                ax[idx].text(x, y+ 0.05, str(round(y,2)),fontweight='bold', ha = "center", fontsize = 12)
        ...
        map_values = np.array([each_log["MAP"] for each_log in self.ap_logs])

        ax2.set_title("MAP per epoch", fontsize = 12)
        ax2.set_ylim(0,1)
        ax2.spines.top.set_visible(False)
        ax2.spines.right.set_visible(False)
        ax2.plot(list(range(len(self.ap_logs))), map_values)

        self.wandb.log(dict(classes_AP_graph = fig))
        self.wandb.log(dict(total_mAP_graph = fig2))
        self.wandb.finish()
        ##############################################################
    ...
    def _add_ground_truth(self, runner):
        ...
        eval_img_idxs = self.eval_image_indexs
        ...
        for idx in self.eval_image_indexs:
            ...
            gt_boxes_list.append(wandb_boxes)

        self.eval_img_gt_index = eval_img_idxs
        self.gt_wandb_boxes = gt_boxes_list
        ...
        for ndx, eval_image_index in enumerate(self.eval_image_indexs):
            ...
            self.eval_table.add_data(
                ...
                self.wandb.Image(
                    ...
                    masks=wandb_masks,
                    classes=self.class_set))
            pred_boxes_list.append(wandb_boxes)
        
        self.eval_img_pred_index = table_idxs
        self.pred_wandb_boxes = pred_boxes_list
        return table_idxs, wandb_boxes
    ...
    def _log_eval_table(self, idx):
        ...
        idx1 = self.eval_img_gt_index 
        idx2 = self.eval_img_pred_index 
        bbox1 = self.gt_wandb_boxes
        bbox2 = self.pred_wandb_boxes

        self.wandb.log(dict(evaluation_data = self.eval_table))

    ###################################
    # Add new feat ####################
    ###################################

    def _make_ground_truth(self, GT_JSON_PATH : str):
        ...
        for image_id in coco.getImgIds():
            ...
            for annotation in annotation_info_list:
                ...
                gt.append([file_name, int(annotation['category_id']),
    ...
    def _make_prediction(self, data, gt_img_id):
        new_pred = []
        print("original data:", len(data))
        for order, each_image in enumerate(data):
            img_id = f"{gt_img_id[order]}"

            for label_idx in range(10):
                predictions = each_image[label_idx]
                for per_prediction in predictions:  
                    if len(per_prediction) != 5 : continue              
                    xmin, ymin, xmax, ymax, confidence = per_prediction.tolist()
                    new_pred.append([img_id, int(label_idx), float(confidence), float(xmin), float(xmax), float(ymin), float(ymax)])
        return new_pred

    def _log_class_map(self, predictions):
        #(TODO) dataset 고정
        json_path = self.val_dataset.ann_file
        label_name = self.val_dataset.CLASSES
        coco, gt, ids = self._make_ground_truth(json_path)
        preds = self._make_prediction(predictions, ids)
        #mAP가 어떻게 계산되는지를 알아야..
        mean_ap, average_precisions = mean_average_precision_for_boxes(gt, preds, iou_threshold=0.5)

        ap_table = self.wandb.Table(columns = ["Label", "Average Precision", "Count"])
        summation = 0
        for key,value in average_precisions.items():
            label = label_name[int(key)]
            ap, cnt = value
            ap_table.add_data(label, ap, cnt)
            summation += int(cnt)
        ap_table.add_data("MAP", mean_ap, summation)

        eval_epoch_dict = {label_name[int(label)]: value[0] for label, value in average_precisions.items()}
        eval_epoch_dict["MAP"] = mean_ap

        return ap_table, eval_epoch_dict
