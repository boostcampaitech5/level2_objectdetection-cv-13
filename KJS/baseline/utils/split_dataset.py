...
def skfold_group_dataset(coco_json, categories, random_seed = 2022):
    ...
    #Make folder first
    output_dir = "./pseudo_groupskfold"
    os.makedirs(output_dir, exist_ok = True)

    new_coco = {}
    cat_ids = coco_json.getCatIds(categories)
    with open("/opt/ml/level2_project/pseudo_dataset/pseudo_filtered_cascade_rcnn_lr_00005_epoch100.json", "rb") as f: 
        data = json.load(f)
...
if SKGFOLD:  
    coco = COCO('/opt/ml/level2_project/pseudo_dataset/pseudo_filtered_cascade_rcnn_lr_00005_epoch100.json')
    skfold_group_dataset(coco, CATEGORIES) 
    
    

if __name__=='__main__':   
    train_counter, val_counter = get_counter('/opt/ml/level2_project/pseudo_dataset', True) 
